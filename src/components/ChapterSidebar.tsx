  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-1 space-y-0.5">
        ...
        >
          <div className="flex items-center gap-2.5">
            <span className={`shrink-0 ${activeChapter === "foreword" ? "text-gold" : "text-muted-foreground group-hover:text-gold-dim"}`}>
              <BookOpen className="w-3.5 h-3.5" />
            </span>
            {!collapsed && (
              <div className="min-w-0">
                <div className={`font-display text-sm leading-tight truncate ${activeChapter === "foreword" ? "text-foreground" : ""}`}>
                  Foreword
                </div>
                <div className="font-mono text-[9px] tracking-wider uppercase text-muted-foreground truncate mt-0.5">
